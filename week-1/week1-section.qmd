...
fracking_clean <- fracking |> 
  ...
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) |> # remove unnecessary columns by filtering for columns of interest
  
  # filter out non-state names ----
  filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill", "Midland", "Red River", "Roosevelt", "Rusk", "State", "WARD")) |> # removing invalid state names
  ...
  mutate(sn = str_to_title(sn)) |> # capitalize state nanes
  ...
  mutate(state_abb = usdata::state2abbr(sn)) |> # simplifies state references
  ...
  drop_na(total_base_water_volume_gal) # filters out empty values 
